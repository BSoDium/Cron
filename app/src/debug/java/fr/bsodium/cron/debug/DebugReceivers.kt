...
import fr.bsodium.cron.debug.receiver.HistorySeedReceiver
import fr.bsodium.cron.debug.receiver.TimelineReproReceiver

/**
 * DEBUG variant — registers debug-only receivers dynamically (`Context.registerReceiver`, not a
 * manifest `<receiver>`) so `adb shell am broadcast` can reach them while the app is backgrounded.
...
object DebugReceivers {
    fun register(context: Context) {
        ...
        ContextCompat.registerReceiver(
            context,
            HistorySeedReceiver(),
            IntentFilter(HistorySeedReceiver.ACTION_TRIGGER_HISTORY_SEED),
            ContextCompat.RECEIVER_EXPORTED,
        )
    }
}
