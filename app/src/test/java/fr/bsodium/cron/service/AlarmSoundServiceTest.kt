...
class AlarmSoundServiceTest {
    ...
    @Test
    fun two_overlapping_alarm_fires_do_not_crash_or_duplicate_the_notification() {
        controller = Robolectric.buildService(
            AlarmSoundService::class.java,
            Intent(app, AlarmSoundService::class.java).apply {
                putExtra(AlarmReceiver.EXTRA_LABEL, "Wake up")
        ...
        // Matches the live AI-alarm-then-hard-latest case (#214): two real fires ~100ms apart.
    ...
    @Test
    fun the_notification_delete_intent_targets_alarm_dismiss() {
        controller = Robolectric.buildService(
            AlarmSoundService::class.java,
            Intent(app, AlarmSoundService::class.java).apply {
                putExtra(AlarmReceiver.EXTRA_LABEL, "Wake up")
                putExtra(AlarmReceiver.EXTRA_REQUEST_CODE, 5)
            },
        )
        controller?.create()?.startCommand(0, 0)
        ...
        val notification = shadowOf(notificationManager).activeNotifications
            ...
            .notification
        val deleteIntent = requireNotNull(notification.deleteIntent)
        assertEquals(AlarmReceiver.ACTION_DISMISS, shadowOf(deleteIntent).savedIntent.action)
    }
