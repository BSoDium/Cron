 ...
 *  track's own calmer color showing through around the anchor. Internal so [TimelineTrackOverlay]
 *  sizes the end-cap disc against it. */
...
internal sealed interface TimelineAnchor {
    ...
    /** The single latest AI run. It shares the regular AI run's `primaryContainer` fill. Only nests at
     *  the cap size/shape when it's genuinely a segment cap — `isLatest` does NOT guarantee
     *  `isSegmentTop`: an `Event` timestamped more recently than the latest AI run (e.g. an alarm
     *  dismissed after last night's plan ran) sorts above it, making the "latest" run an interior row
     *  despite the name. `TimelineNode` normalizes it to a plain interior `Icon` in that case — see
     *  `effectiveAnchor`. */
...
internal fun TimelineNode(
    ...
    /** An interior Icon anchor drops its semantic accent entirely — shape and color both fall back to
     *  the track-matched treatment below, rather than threading an `atCap` check separately through
     *  the shape `when`, the socket color, and the glyph tint; `socketAccentColor`'s
     *  `containerColor ?: trackAccentColor(...)` fallback just does the right thing without knowing
     *  about `atCap` itself. A non-cap Latest anchor (see its own KDoc — `isLatest` doesn't guarantee
     *  `isSegmentTop`) is converted into a plain interior Icon for the same reason: "only cap anchors
     *  render as a Circle" applies as much to Latest as to a regular Icon, so it's normalized through
     *  the identical Icon path. */
    ...
    /** Only two shapes exist: a cap anchor (segment top/bottom, or a sleep sub-track's own onset/wake)
     *  is always [AnchorShape.Circle]; every interior anchor is always [AnchorShape.Pill]. No press or
     *  arrival animation changes this — see docs/expressive.md. */
    val shape = if (atCap) AnchorShape.Circle else AnchorShape.Pill
     ...
     *  reported into the registry alongside the outgoing shape so the overlay can blend the two
     *  draws. */
    ...
    Box(modifier = modifier.fillMaxWidth()) {
        if (onClick != null) {
            Surface(
                ...
                // Bleeds the tap/ripple target to the true screen edges past HomeContent's LazyColumn contentPadding (compensating padding below keeps inner() in place).
                modifier = Modifier
                    .fillMaxWidth()
