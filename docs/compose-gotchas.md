...
- **Never hardcode `TopAppBarState`'s constructor args from scratch — always start from `rememberTopAppBarState()`'s own defaults.** The three params are `(initialHeightOffsetLimit, initialHeightOffset, initialContentOffset)`, and the library's real defaults are `(-Float.MAX_VALUE, 0f, 0f)` — **not** `(0f, 0f, 0f)` or any other "looks like a sane zero" guess. `heightOffsetLimit == 0f` is a real, distinct state the collapse-fraction math checks for, not a harmless placeholder: with a `0f` initial limit, `LargeFlexibleTopAppBar`'s collapsed small-title row renders with zero content — title text is silently invisible once the bar fully collapses, with no error, exception, or lint warning (found and fixed in the Memory-tab PR — `MainActivity.kt`'s and `SettingsScreen.kt`'s hoisted `TopAppBarState`s both hardcoded `(0f, 0f, 1f)`). Confirmed via a scrolled-state Roborazzi screenshot (`SettingsScreenScreenshotTest`), not visible from an unscrolled `@Preview`. If you need to hoist this state (e.g. so a predictive-back preview reflects live collapse), write `TopAppBarState(-Float.MAX_VALUE, 0f, 0f)` explicitly, matching `rememberTopAppBarState()`'s signature default-for-default.
